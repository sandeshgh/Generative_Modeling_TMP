...
input_dim = 1898
mode=1
seg_index=np.array([100])
exc_index=np.array([85,185,285,585,785,985,1085,1185,1385,1585,1685,1785,1885])
#seg_index=np.array([[1,2],[2,8],[3,9],[8,9],[3,4],[5,6],[1,7],[4,10],[5,11],[6,12],[7,12],[10,11],[6,11]])
...
def readU(path,seg_in,exc_index=71):
    ...
    if (seg_in.size >1):
        ...
        TrainData = sio.loadmat(path + 'AwTmpSeg' + str(seg_in)  + 'exc' + str(exc_index) + '.mat')
    Ut = (TrainData['U'])
    (a,b)=Ut.shape
    if a==input_dim:
        U=Ut
...
def main():
    ...
    # ----- Read H,Y,beta-----
    pathH = '/Users/sg9872/Desktop/Research/Halifax-Processed/Simulation/1898/Input/'
    pathZ = '/Users/sg9872/Desktop/Research_Projects/Sequence_VAE/AW1898/ExperimentData/Latent/'
    if mode==2:
        pathU='/Users/sg9872/Desktop/Research_Projects/Sequence_VAE/AW1898/ExperimentData/ScarUnseenExc/'
    else:
        pathU = '/Users/sg9872/Desktop/Research_Projects/Sequence_VAE/AW1898/ExperimentData/Healthy/'
    ...
    for i in range(6,7):
        ...
        #print('Y is :', H @ U)
        Y = genNoisy(H @ U)
        print('Sample {} running'.format(i))

        #----Sampling part
        z0 = np.load(pathZ + 'ZSeg100avg.npy')
        #zvar=np.load(pathZ + 'VarSeg100avg.npy')
        #if j<2:
        #    z0 = np.load(pathZ + 'ZSeg100exc441.npy')

        #else:
         #   z0 = np.load(pathZ+'ZSeg100exc481.npy')
        #z0 = np.load(pathZ+'ZSeg100exc481.npy')
        z0=torch.FloatTensor(z0).view(seq_length,1,-1)
        #zvar=torch.FloatTensor(zvar).view(seq_length,1,-1)
        #z_prec=zvar.reciprocal()
        ...
        for k in range(40):
            ...
            #er2= torch.sum(Variable(z_prec)*((Z-Variable(z0)).pow(2)))
