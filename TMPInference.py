...
def calculate_loss(MuZ,logSigmaZ,Upos,H,Y,beta):
    (d,n)=Y.size()
    diffSq = (Upos- MuZ).pow(2)
    precis = torch.exp(-logSigmaZ)
    lossZ = 0.5 * torch.sum(logSigmaZ + torch.mul(diffSq, precis))
    lossZ/= (input_dim * seq_length)

    datafit=torch.sum(Y-H@Upos).pow(2)
    lossY=-0.5*(d*torch.log(beta)-beta*datafit)
    lossY/=(d*seq_length)
    return lossY+lossZ
...
def main():
    ...
    z1=Variable(torch.randn([seq_length, 1, 50]))

    mu1, logvar1 = model.decode(z1)  # Converting into torch variable and decoding
    sigma1 = logvar1.exp()
    mu1 = (mu1.data.view(seq_length, -1))
    sigma1 = (sigma1.data.view(seq_length, -1))
    muz = mu1.transpose(0, 1)
    sigmaz = sigma1.transpose(0, 1)

    Uinit = Posterior(muz, sigmaz, H, Y, beta)
    # ------Reading ends------
    ...
    for j in range(n_samples):
        ...
        Upos=Variable(Uinit,requires_grad=True)
        optimizer = optim.Adam([Z,Upos], lr=1e-3)
        for k in range(100):
            Mu,logvar=model.decode(Z) #Converting into torch variable and decoding
            Sigma=logvar
            Mu = (Mu.data.view(seq_length, -1))
            Sigma = (Sigma.data.view(seq_length, -1))
            MuZ=Mu.transpose(0,1)
            logSigmaZ=Sigma.transpose(0,1)
            #Sampling ends---------
            ...
            optimizer.zero_grad()
            loss = calculate_loss(Variable(MuZ),Variable(logSigmaZ),Upos,Variable(H),Variable(Y),(beta))
            loss.backward()
            optimizer.step()

            MeanU=Upos.data
            errorU=torch.norm(MeanU-U)
    ...
    plotMatrix(winner_U.numpy(), 'winner', 1406, 1410)
    sio.savemat('Winner_U.mat', {"U":winner_U.numpy()})
