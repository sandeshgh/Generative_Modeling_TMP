...
def calculate_loss(Mu,logvar,Upos,Ht,Yt,beta):
    (a,b,d)=Yt.size()
    Upred=Variable((Upos.data.view(-1,input_dim)@Ht.data).view(seq_length,1,-1))
    difData=(Yt-Upred).pow(2)
    #print('difData is',difData)
    #print('beta is', beta)
    lossY=0.5*beta*torch.sum(difData)
    lossY/=(d*seq_length)
    #print('Upos is',Upos)
    #print('Mu is',Mu)
...
def main():
    ...
    Ht=Variable(H.transpose(0,1))
    Yt=Variable(Y.transpose(0, 1).contiguous().view(seq_length, 1, -1))
    ...
    for j in range(n_samples):
        ...
        U_pos = Variable(U_pos.transpose(0, 1).contiguous().view(seq_length, 1, -1),requires_grad=True)

        optimizer = optim.Adam([Z,U_pos], lr=1e-2)
        for k in range(20):
            ...
            #for l in range(5):
            optimizer.zero_grad()
            loss = calculate_loss((Mu),(logvar),U_pos,Ht,Yt,Variable(beta))
            loss.backward()
            optimizer.step()
            Mu, logvar = model.decode(Z)

            errorU=torch.norm(U_pos-Variable(U.transpose(0, 1).contiguous().view(seq_length, 1, -1)))
