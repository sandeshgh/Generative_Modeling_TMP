...
def loss_function(muTheta,logvarTheta, x, mu, logvar):
    #tol=1e-8
    #BCE = -torch.sum(torch.mul(x,torch.log(tol+recon_x))+(1-x).mul(torch.log(1+tol-recon_x)))
    diffSq=(x-muTheta).pow(2)
    BCE=0.5*torch.sum(logvarTheta+torch.mul(diffSq,torch.reciprocal(logvarTheta.exp())))
    ...
    KLD /= batch_size * input_dim*seq_length
...
def train(epoch):
    ...
    while j < input_dim:
        ...
        for i in seg_range:
            ...
            U=U.resize_((seq_length,input_dim,1))
            #print(U)
            if i == 0:
                outData = U
            elif i > 0 and i < 17:
                outData = torch.cat((outData, U), 2)

        data = Variable(outData.permute(0,2,1))  #sequence length, batch size, input size
        #print(data.size())
        if args.cuda:
            data = data.cuda()
        optimizer.zero_grad()
        muTheta,logvarTheta, mu, logvar = model(data)
        loss, KL, CE = loss_function(muTheta,logvarTheta, data, mu, logvar)
        KLD+=KL
        CE_loss+=CE
        loss.backward()
    ...
    print('====> Epoch: {} Average loss: {:.4f}, KLD: {}, CELoss:{}'.format(
          epoch, train_loss / n_samples,KLD/n_samples,CE_loss/n_samples))
    return train_loss, KLD,CE_loss
...
for epoch in range(1, args.epochs + 1):
    model.train()
    loss,kld,ce=train(epoch)
    Loss_array=[Loss_array,loss]
    KLD_array=[KLD_array,kld]
    CE_array=[CE_array,ce]
...
plt.plot(Loss_array.numpy())
plt.hold('on')
plt.plot(KLD_array.numpy(),'r')
plt.hold('on')
plt.plot(CE_array.numpy(),'g')
plt.hold('off')
plt.show()
'''
    test(epoch)
