...
if modelStructure=='SeqGauss':
    ...
    if dataset=='AW1898':
        ...
        modelfull = torch.load('output/modelLatentsmallFull600', map_location={'cuda:0': 'cpu'})
        modelfull2 = torch.load('output/modelLatentsmallFull1000', map_location={'cuda:0': 'cpu'})
        real_path = os.getcwd()
        os.chdir("../EC1862/BigData")
        path_data = os.getcwd()
        os.chdir(real_path)
        path_out="/Users/sg9872/Desktop/Research_Projects/Sequence_VAE/AnalyzeVisual/EC1862/Small/"
        fileName='Seg'+str(i)+'exc' + str(j)
    ...
    for alpha in range(3,4):
        sampleNew=Z;
        sampleNew[:,(range(1,5))]=(alpha-3)*0.25*Z[:,(range(1,5))]
