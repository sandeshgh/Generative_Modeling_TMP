...
def Likelihood_Y_Z(MuZ,SigmaZ,H,Y,beta):
    ...
    for i in range(N):
        ...
        #outerProduct=torch.ger(deviation,deviation)
        Sigmai=torch.diag(SigmaZ[:,i])
        Covi=(H@Sigmai)@H.transpose(0,1)+(1/beta)*Variable(torch.eye(d))
        Pr=torch.inverse(Covi)
        expTerm=torch.dot(deviation,(Pr@deviation))
        #logdet=torch.log(torch.det(Pr))
        #print('Determinant is:', determin)
        #print(Covi)
        #prod*=1/(math.exp(0.5*expTerm)*math.sqrt(lin.det(Covi)))
        totalTerm=totalTerm+0.5*(-expTerm)
...
def main():
    ...
    for j in range(n_samples):
        ...
        for k in range(20):
            ...
            #for l in range(5):
            optimizer.zero_grad()
            loss = Likelihood_Y_Z(Variable(MuZ),Variable(SigmaZ),Variable(H),Variable(Y),Variable(beta))
            loss.backward()
            optimizer.step()
            Mu, logvar = model.decode(Z)
